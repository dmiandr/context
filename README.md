# Context RepuTracker
Индивидуальный менеджер репутаций пользователей социальных сетей, оформленный как расширение для браузера (webextension), поддерживаются полноразмерные (не мобильные) браузеры, созданные на базе Chrome и Mozilla.
